...
def test_iteration_time_zero_or_less(iteration):
    with pytest.raises(
        ValueError,
        match=match(
            "The duration of the metronome iteration (tick-tock time) must be greater than zero."
        ),
    ):
...
def test_alternation_of_sleep_and_callback():
    ...
    metronome = Metronome(
        0.1,
        lambda: (actions.append(1), sleep(0.0001)),
        sleeping_callback=lambda x: actions.append(2),
    )
    metronome.start()
...
def test_start_started():
    ...
    with pytest.raises(
        RunAlreadyStartedMetronomeError,
        match=match("The metronome has already been launched."),
    ):
        metronome.start()
...
def test_stop_stopped():
    ...
    with pytest.raises(
        StopStoppedMetronomeError,
        match=match(
            "You've already stopped this metronome (or it was canceled on its own, for example, when the limit expired), it's impossible to do it twice."
        ),
    ):
        metronome.stop()
...
def test_start_stopped():
    ...
    with pytest.raises(
        RunStoppedMetronomeError,
        match=match(
            "Metronomes are disposable, you cannot restart a stopped metronome."
        ),
    ):
        metronome.start()
...
def test_stop_not_started():
    ...
    with pytest.raises(
        StopNotStartedMetronomeError,
        match=match("You can't stop a metronome that hasn't been started yet."),
    ):
        metronome.stop()
...
def test_normal_logs_order():
    ...
    for index, log_item in enumerate(copy(logger.data.debug)):
        if index % 2 == 0:
            assert (
                log_item.message
                == 'The beginning of the execution of callback "callback".'
            )
        else:
            assert (
                log_item.message
                == 'Callback "callback" has been successfully completed.'
            )
...
def test_start_with_cancelled_token(first_token, second_token):
    ...
    logger = MemoryLogger()
    metronome = Metronome(
        0.0001, lambda: actions.append(True), token=first_token, logger=logger
    )
    ...
    assert logger.data.info[0].message == "The metronome starts..."
    assert (
        logger.data.info[1].message
        == "The metronome did not start working because the cancellation token was canceled right at the start."
    )
...
def test_behavior_when_the_callback_time_is_bigger_than_loop_time():
    ...
    def callback():
        (sleep(sleep_time), actions.append(1))
    ...
    for log_item in logger.data.warning:
        assert log_item.message.startswith(
            "The callback worked for more than the amount of time allocated for one iteration. The extra time was "
        )
...
def test_iteration_more_than_duration():
    with pytest.raises(
        ValueError,
        match=match(
            "The time of one iteration cannot exceed the running time of the metronome as a whole."
        ),
    ):
...
def test_duration_less_than_zero():
    with pytest.raises(
        ValueError,
        match=match(
            "The total duration of the metronome operation cannot be less than zero."
        ),
    ):
...
def test_raise_into_context_manager():
    ...
    with pytest.raises(ValueError, match=match("text")):
        with metronome:
